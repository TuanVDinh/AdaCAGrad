...
def multi_task_mgd_trainer(train_loader, test_loader, multi_task_model, device,
    ...
    best_acc_val = -1
    # prev_net = copy.deepcopy(multi_task_model)
    # prev_net.to(device)
    for index in range(total_epoch):
        ...
        total = 0.0
        losses = 0.0
        sigma = 0.5 #np.random.rand(1)
        kappa = 0.75 #np.random.rand(1)
        #sigma = np.random.rand(1)[0]
        #kappa = np.random.rand(1)[0]
        ...
        for k in range(train_batch):
            ...
            prev_loss = np.array([train_loss_tmp[0].item(),train_loss_tmp[1].item()])
            ...
            if method == "graddrop":
                ...
                optimizer.step()
            elif method == "single_L":
                train_loss_tmp[0].backward()
                optimizer.step()
            elif method == "single_R":
                train_loss_tmp[1].backward()
                optimizer.step()
            elif method == "mgd":
                for i in range(2):
                    if i < 2:
                ...
                prev_net = copy.deepcopy(multi_task_model)
                prev_net.to(device)
                task1_cur, task2_cur = prev_net(img)

                train_loss_tmp_c = [nn.CrossEntropyLoss()(task1_cur,ys[:, 0]),nn.CrossEntropyLoss()(task2_cur,ys[:, 1])]
                cur_loss = np.array([train_loss_tmp_c[0].item(),train_loss_tmp_c[1].item()])
                g_norm = g.unsqueeze(0)
                # print((w*prev_loss).sum())
                # print((w*cur_loss).sum())
                # print("check: ",g_norm.mm(g_norm.t()))
                # print(sigma * optimizer.param_groups[0]['lr']*((((g_norm.mm(g_norm.t())))**2).detach().cpu().numpy()))
                # print(((w*prev_loss).sum() + sigma * optimizer.param_groups[0]['lr']*((((g_norm.mm(g_norm.t())).norm())**2).detach().cpu().numpy())) - (w*cur_loss).sum())  
                VT = (w*cur_loss).sum()
                VP = ((w*prev_loss).sum() - sigma * optimizer.param_groups[0]['lr']*((((g_norm.mm(g_norm.t())).norm())**2).detach().cpu().numpy()))  
                if  (VP - VT) < 0:
                    if np.abs(VP - VT) > 0.1:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            mean_loss = (l1_train + l2_train)/2
            log_dict = {
                    "Mean accuracy valid": mean_acc,
                    "Mean loss train": mean_loss,
                    "Learning rate":optimizer.param_groups[0]['lr'],
                }
            wandb.log(log_dict)
            print('Epoch: {:04d},Mean_acc: {:.4f}, Acc_val_task1: {:.4f}, Acc_val_task2: {:.4f}, Loss_train_task1: {:.4f}, Loss_train_task2: {:.4f}'.format(index,mean_acc,task1_correct.cpu().item() / total,task2_correct.cpu().item() / total,l1_train,l2_train))
            if mean_acc > best_acc_val:
                print("Save model")
                best_acc_val = mean_acc
                if "cagrad" in method or "adacagrad" in method:
                    torch.save(multi_task_model.state_dict(), f"models/{dataset_name}_{method}-{opt.weight}-{alpha}-{seed}.pt")
                    if "adacagrad" in method:
                        torch.save(multi_task_model.state_dict(), f"models/{dataset_name}_{method}-{opt.weight}-{alpha}-{opt.kappa}-{seed}.pt")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kappa', default=0.75, type=float, help='the kappa')
